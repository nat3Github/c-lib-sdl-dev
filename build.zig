...
pub fn build(b: *std.Build) void {
    ...
    switch (t.os.tag) {
        ...
        .macos => {
            ...
            if (system_library_path) |p| lib.root_module.addLibraryPath(p);
            if (builtin.os.tag != .macos and (system_include_path == null or system_framework_path == null or system_library_path == null)) {
        ...
        switch (t.os.tag) {
            .linux => {
                ...
                if (cross_linux) {
                    if (system_include_path) |p| lib.root_module.addSystemIncludePath(p);
                    if (system_library_path) |p| lib.root_module.addLibraryPath(p);
                    if (system_include_path == null or system_library_path == null) {
                        std.debug.print("error: cross-compiling to Linux requires -Dinclude_path and -Dlibrary_path pointing at a Linux sysroot's usr/include and usr/lib (X11/pulse headers+libs)\n", .{});
                        std.process.exit(1);
                    }
                }
